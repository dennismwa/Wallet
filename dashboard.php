...
@media (max-width: 640px) {
    ...
    #notifications-dropdown.show-mobile::before {
        content: '';
        position: fixed;
        top: 0;
        left: 0;
...
/* Improved notification item spacing */
.notification-item {
    @apply p-4 border-b;
}
            ...
            <div class="flex items-center justify-between px-6 py-4">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="relative">
                        ...
                        <div id="notifications-dropdown" class="absolute right-0 mt-2 w-80 sm:w-80 <?= $darkMode ? 'bg-gray-800' : 'bg-white' ?> rounded-lg shadow-xl border <?= $darkMode ? 'border-gray-700' : 'border-gray-200' ?> z-50 hidden">
                            
                            <!-- Header -->
                            <div class="p-4 border-b <?= $darkMode ? 'border-gray-700' : 'border-gray-200' ?> flex items-center justify-between">
                                <h3 class="font-semibold <?= $darkMode ? 'text-white' : 'text-gray-800' ?>">Notifications</h3>
                                <!-- Close button for mobile -->
                                <button onclick="closeNotifications()" class="sm:hidden <?= $darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-700' ?> p-1">
                            ...
                            <div class="max-h-80 sm:max-h-96 overflow-y-auto">
                                <?php if (empty($notifications)): ?>
                                    <div class="p-6 text-center <?= $darkMode ? 'text-gray-400' : 'text-gray-500' ?>">
                                ...
                                <?php else: ?>
                                    <?php foreach ($notifications as $notification): ?>
                                        <div class="notification-item p-4 border-b last:border-b-0 <?= $darkMode ? 'border-gray-700' : 'border-gray-200' ?> <?= $darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50' ?> transition-colors" data-notification-id="<?= $notification['id'] ?>">
                                            <div class="flex items-start space-x-3">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <h4 class="font-medium <?= $darkMode ? 'text-white' : 'text-gray-800' ?> text-sm leading-5">
                                                        <?= htmlspecialchars($notification['title']) ?>
                                                    </h4>
                                                    <p class="notification-message text-sm <?= $darkMode ? 'text-gray-400' : 'text-gray-600' ?> mt-1 leading-5 break-words">
                                                        <?= htmlspecialchars($notification['message']) ?>
                                                    </p>
                                                    <p class="text-xs <?= $darkMode ? 'text-gray-500' : 'text-gray-400' ?> mt-2">
                                                        <?= formatDate($notification['created_at'], 'M j, Y g:i A') ?>
                                                    </p>
                                                </div>
                                                
                                                <!-- Action Menu -->
                                                <div class="flex-shrink-0">
                                                    <button onclick="markAsRead(<?= $notification['id'] ?>)" class="<?= $darkMode ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-600' ?> p-1 transition-colors" title="Mark as read">
                            ...
                            <!-- Footer Actions -->
                            <?php if (!empty($notifications)): ?>
                                <div class="p-4 border-t <?= $darkMode ? 'border-gray-700' : 'border-gray-200' ?> <?= $darkMode ? 'bg-gray-700' : 'bg-gray-50' ?> rounded-b-lg">
                                    <div class="flex items-center justify-between">
                                        <button onclick="markAllAsRead()" class="text-blue-600 hover:text-blue-700 text-sm font-medium transition-colors">
                                            Mark all as read
                                        </button>
                                        <a href="notifications.php" class="text-blue-600 hover:text-blue-700 text-sm font-medium transition-colors">
                                            View all
                                        </a>
                                    </div>
                                </div>
...
// Toggle notifications with mobile considerations
function toggleNotifications(e) {
    ...
    if (isHidden) {
        ...
        // Add mobile-specific class on small screens
        if (window.innerWidth <= 640) {
            notificationsDropdown.classList.add('show-mobile');
...
function closeNotifications() {
    ...
    notificationsDropdown.classList.remove('show-mobile');
...
function markAsRead(notificationId) {
    ...
    .then(result => {
        if (result.success) {
            ...
            const notificationElement = event.target.closest('.notification-item') || event.target.closest('[data-notification-id="' + notificationId + '"]');
...
function updateNotificationBadge() {
    const remainingNotifications = document.querySelectorAll('#notifications-dropdown .notification-item').length;
...
notificationsDropdown.addEventListener('touchend', e => {
    ...
    // Close on swipe up (if swiped more than 100px)
