    ...
    if ($is_external_method && $use_external_payment) {
        ...
        $transactionId = $db->execute(
            "INSERT INTO transactions (user_id, category_id, type, amount, description, payment_method, balance_after, transaction_date, status, reference_number, notes) 
             VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), 'pending', ?, ?)",
            [$user_id, $category_id, $type, $amount, $description, $payment_method, $balance_after, $reference_number, $notes]
        );
        ...
        $transactionId = $db->execute(
            ...
            [$user_id, $category_id, $type, $amount, $description, $payment_method, $newBalance, $reference_number, $notes]
        );
        ...
        if ($amount >= 10000) { // Configurable threshold
            $db->execute(
                ...
                [
                    ...
                    "Large {$type} of " . formatMoney($amount, 'KES') . " recorded: {$description}",
